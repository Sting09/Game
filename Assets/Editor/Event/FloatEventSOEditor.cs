...
{
    private FloatEventSO floatEventSO;

    private void OnEnable()
    {
        floatEventSO = target as FloatEventSO;
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        // 显示最后发送者信息
        if (!string.IsNullOrEmpty(floatEventSO.lastSender))
        {
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("最后发送者：", EditorStyles.boldLabel);
            EditorGUILayout.LabelField(floatEventSO.lastSender, EditorStyles.miniLabel);
        }
    }
    ...
    private List<MonoBehaviour> GetListeners()
    {
        ...
        if (floatEventSO == null || floatEventSO.OnEventRaised == null)
        {
            return listeners;
        }
        ...
        try
        {
            var subscribers = floatEventSO.OnEventRaised.GetInvocationList();
