...
public class ObjectEventSOEditor : Editor
{
    private ObjectEventSO objectEventSO;

    private void OnEnable()
    {
        objectEventSO = target as ObjectEventSO;
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        // 显示最后发送者信息
        if (!string.IsNullOrEmpty(objectEventSO.lastSender))
        {
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("最后发送者：", EditorStyles.boldLabel);
            EditorGUILayout.LabelField(objectEventSO.lastSender, EditorStyles.miniLabel);
        }
    }
    ...
    private List<MonoBehaviour> GetListeners()
    {
        ...
        if(objectEventSO == null || objectEventSO.OnEventRaised == null)
        {
            return listeners;
        }
        ...
        try
        {
            var subscribers = objectEventSO.OnEventRaised.GetInvocationList();
