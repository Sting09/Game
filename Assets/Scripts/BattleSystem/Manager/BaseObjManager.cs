...
public abstract class BaseObjManager<T> : SingletonMono<T> where T : BaseObjManager<T>
{
    ...
    /// <summary>
    /// Handles battle-end cleanup for DOTS/job objects before leaving the battle scene.
    /// </summary>
    public void CleanupBeforeLeavingBattle()
    {
        if (!m_IsInitialized) return;

        m_JobHandle.Complete();
        OnBeforeCleanupBeforeLeavingBattle();

        for (int i = m_ActiveGOs.Count - 1; i >= 0; i--)
        {
            GameObject activeObj = m_ActiveGOs[i];
            int visualID = m_ActiveVisualIDs[i];

            if (activeObj != null)
            {
                activeObj.SetActive(false);
                if (m_VisualPools != null && visualID >= 0 && visualID < m_VisualPools.Length && m_VisualPools[visualID] != null)
                {
                    m_VisualPools[visualID].Enqueue(activeObj);
                }
                else
                {
                    Destroy(activeObj);
        ...
        if (m_CollisionQueue.IsCreated)
        {
            while (m_CollisionQueue.TryDequeue(out _)) { }
        }

        m_ActiveEmitters.Clear();
        m_LastEmitterPos.Clear();
        m_EmittersToRemoveCache.Clear();
        m_DeadEmittersThisFrame.Clear();
        if (m_EmitterDeltas.IsCreated)
        {
            m_EmitterDeltas.Clear();
        }
        ...
        OnAfterCleanupBeforeLeavingBattle();
    }

    /// <summary>
    /// Allows subclasses to complete/clear any custom job data before base cleanup runs.
    /// </summary>
    protected virtual void OnBeforeCleanupBeforeLeavingBattle()
    {
    }

    /// <summary>
    /// Allows subclasses to clear pending buffers after base cleanup finishes.
    /// </summary>
    protected virtual void OnAfterCleanupBeforeLeavingBattle()
    {
    }
