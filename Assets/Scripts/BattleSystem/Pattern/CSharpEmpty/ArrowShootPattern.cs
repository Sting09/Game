        ...
        for(int i = 0; i < bulletsPerWay; i++)
        {
            ...
            for (int k = 0; k <= n; k++)
            {
                if (k == 0)    //正中间的子弹特别处理
                {
                    ...
                    info.lifetime = 0;
                    // 最终位置 = 发射器位置 + 箭头形状偏移
                    BulletDOTSManager.Instance.AddBullet(bulletName, pos + CalculatePosOffset(dir, distance), info);
                ...
                infoLeft.lifetime = 0;
                BulletDOTSManager.Instance.AddBullet(bulletName, pos + CalculatePosOffset(dir + angleBOAkDeg, distance), infoLeft);
                ...
                BulletDOTSManager.Instance.AddBullet(bulletName, pos + CalculatePosOffset(dir - angleBOAkDeg, distance), infoRight);
